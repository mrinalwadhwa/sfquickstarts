...
summary: A guide to building and running a custom private API Powered by Snowflake and Python/Flask
...
# Build a private Custom API in Python and Flask

<!-- ------------------------ -->
## Overview
Duration: 5

Many builders want to share some of the data stored in Snowflake over an http API. Enterprise applications often want to retrieve that data through http APIs. This tutorial will go through how to build, deploy, and host a private custom API Powered by Snowflake.

This API will not have an endpoint that is exposed to the Internet. Your application will, instead, access this API over private endpoints that are only available within your enterprise's VPC and other private environements.
